...
import com.zxjaihhl.yds.adapter.HeaderAdAdapter;
import com.zxjaihhl.yds.manager.ImageManager;
import com.zxjaihhl.yds.utils.DensityUtil;
...
public class HeaderAdViewView extends HeaderViewInterface<List<String>> {
    ...
    protected void getLocalView(List<String> list, ListView listView) {
        View view = mInflate.inflate(R.layout.header_ad_layout, listView, false);
        ButterKnife.bind(this, view);

        dealWithTheLocalView(list);
        listView.addHeaderView(view);
    }
    ...
    private void dealWithTheLocalView(List<String> list) {
        ivList.clear();
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ivList.add(createImageView(list.get(i)));
    ...
    // 创建要显示的drawble ImageView
    private ImageView createDrawbleImageView(int resId) {
        ImageView imageView = new ImageView(mContext);
        AbsListView.LayoutParams params = new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        imageView.setLayoutParams(params);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        mImageManager.loadResImage(resId, imageView);
        return imageView;
    }
    // 创建要显示的local ImageView
    private ImageView createLocalImageView(String url) {
        ImageView imageView = new ImageView(mContext);
        AbsListView.LayoutParams params = new AbsListView.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT);
        imageView.setLayoutParams(params);
        imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
        mImageManager.loadLocalImage(url, imageView);
        return imageView;
    }
