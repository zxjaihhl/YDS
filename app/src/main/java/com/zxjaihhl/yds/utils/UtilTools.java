...
public class UtilTools {
    //设置字体
    public  static void setlFont(Context mContext, TextView textView){
        Typeface fontType= Typeface.createFromAsset(mContext.getAssets(),"fonts/苹果丽黑.ttf");
    ...
    //读取图片
    public static Bitmap getImage(Context mContext) {
        //1.拿到string
        String imgString = SharedUtils.getString(mContext, "image_title", "");
        Bitmap bitmap = null;
        if (!imgString.equals("")) {
            ...
            ByteArrayInputStream byStream = new ByteArrayInputStream(byteArray);
            //3.生成bitmap
            bitmap = BitmapFactory.decodeStream(byStream);
        }
        return bitmap;
    }
//    //获取版本号
